...
This module is the seed interop layer between Codex and a running Microsoft
Word desktop instance. Public commands wrap first-class Word COM operations in
an interface that is discoverable with ordinary PowerShell tooling.
...
[`Bespoke`](Bespoke). They may depend on public commands, but never on private
functions. The full dependency and promotion model is documented in
